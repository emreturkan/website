  ...
  const getSteam = async () => {
    await fetch("/api/getSteam")
      ...
      .then(async (data) => {
        ...
        await fetch(`/api/getSteamGameDetails?appId=${mostRecentGame.appid}`)
          .then((res) => res.json())
          ...
          {loading && (
            <SkeletonTheme baseColor="#202020" highlightColor="#444">
              <Skeleton
                ...
                className="h-20 w-32 mt-5 md:w-[300px] rounded-md"
              />
            </SkeletonTheme>
          )}
          {!loading && !gameData && (
